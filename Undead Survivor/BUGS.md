...
### ~~[IMPROVE-004] 플레이어 사망 시 게임 상태 미처리~~ ✅ 해결
- `Player.OnCollisionStay2D()`에서 `GameManager.instance.GameOver()` 호출로 해결

### [IMPROVE-005] 불필요한 using 선언
- **파일**: `Assets/Undead Survivor/code/Player.cs:3`, `GameManager.cs:3`
- **증상**: `using Unity.VisualScripting;` 자동 추가됐으나 실제로 사용되지 않음
- **수정 방법**: 두 파일에서 해당 using 제거
